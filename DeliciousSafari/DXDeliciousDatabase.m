...
static NSString* const kOAuthAccessTokenKeyKey = @"OAuthAccessTokenKeyKey";
static NSString* const kOAuthAccessTokenSecretKey = @"OAuthAccessTokenSecretKey";
static NSString* const kOAuthSessionHandle = @"OAuthSessionHandle";
...
-(void)dropAllTables
{
	...
	for(i = 0; i < count; ++i)
	{
		...
		@catch (NSException * e) {
			NSLog(@"Error dropping %s table. %@", tables[i], e);
		}
	}
}
...
-(NSArray*)postsForTagArray:(NSArray*)tagArray orderBy:(NSString*)orderByClause
{
	NSString *sqlFragmentFormatString = @"SELECT *, lower(title) FROM bookmark WHERE bookmark.id IN (SELECT bookmark_id FROM bookmark_tag, tag WHERE bookmark_tag.tag_id = tag.id and tag.name_lower = ?%d)";
...
-(OAToken*)oauthAccessToken
{
	NSString* key = [self stringForKey:kOAuthAccessTokenKeyKey];
	NSString* secret = [self stringForKey:kOAuthAccessTokenSecretKey];
	
	OAToken *result = nil;
	
	if (key && secret && [key length] > 0 && [secret length] > 0)
	{
		result = [[[OAToken alloc] initWithKey:key secret:secret] autorelease];
	}
	
	return result;
}

-(void)setOAuthAccessToken:(OAToken*)accessToken
{
	NSString *key = [accessToken key];
	...
	[self setString:key forKey:kOAuthAccessTokenKeyKey];
	[self setString:secret forKey:kOAuthAccessTokenSecretKey];
}

-(NSString*)oauthSessionHandle
{
	return [self stringForKey:kOAuthSessionHandle];
}

-(void)setOAuthSessionHandle:(NSString*)sessionHandle
{
	[self setString:sessionHandle forKey:kOAuthSessionHandle];
}
...
-(BOOL)isUsingOAuth
{
	return [self oauthAccessToken] != nil;
}
