...
@synthesize yahooOAuthManager=mYahooOAuthManager;
...
+(DXDeliciousAPI*)sharedInstance
{
	...
	if(dsAPI == nil)
	{
		...
		NSString *agentName = @"DeliciousSafari";
		...
		dsAPI.yahooOAuthManager = [DXYahooOAuthManager sharedOAuth];
	}
...
-(id)initWithUserAgent:(NSString*)userAgent
{
	if([super init])
	{
		...
		mBaseOAuthURL = [[NSURL alloc] initWithString:@"http://api.del.icio.us/v2/"];
...
-(void)dealloc
{
	mDelegate = nil;
	[mBaseURL release];
	[mBaseOAuthURL release];
	[mBaseJSONURL release];
	[mDateStampFormatter release];
	[mUserAgent release];
	[mYahooOAuthManager release];
...
- (void)startRequestToURL:(NSString*)urlFragment withResponse:(SEL)selector
{
	NSURL *baseURL = self.isUsingOAuth ? mBaseOAuthURL : mBaseURL;
	[self startRequestToURL:urlFragment withResponse:selector isJSONRequest:NO relativeToBase:baseURL];
}
...
{	
	...
	NSLog(@"delayedStartRequestToURL called at %@ for url %@", [NSDate date], [theRequest URL]);
	...
	{
		NSMutableURLRequest *requestToUse = theRequest;
		BOOL useOAuth = NO;
		
		if (self.isUsingOAuth)
		{
			requestToUse = [self.yahooOAuthManager oaRequestFromURLRequest:theRequest];
			useOAuth = YES;
			assert(requestToUse);
		}
		
		// theConnection is released later on during connection processing.
		DXURLConnection *theConnection = [[DXURLConnection alloc] initWithRequest:requestToUse
																		 delegate:self
															 withResponseSelector:selector
														 withExpectingXMLResponse:[expectingXMLResponse boolValue]
																	 isUsingOAuth:useOAuth];
...
- (void)connection:(NSURLConnection *)connectionObj didReceiveResponse:(NSURLResponse *)response
{
	...
	// For Yahoo OAuth meanings, see http://developer.yahoo.com/oauth/guide/oauth-errors.html
	const int kHTTP_BadRequest = 400;
	const int kHTTP_Unauthorized = 401;
	...
	if([response isKindOfClass:[NSHTTPURLResponse class]])
	{
		...
		//NSLog(@"Received data length: %d", [[connection receivedData] length]);
	}
	...
	else if (statusCode != kHTTP_OK && self.isUsingOAuth)
	{
		[connection cancel];
		
		if (statusCode == kHTTP_BadRequest)
		{
			// Look at the response to figure out what to do. If token_expired then refresh the access token.
			NSLog(@"Data received: %s", [[connection receivedData] bytes]);
			
			// TODO: Do this async.
			[self.yahooOAuthManager refreshAccessToken];
			[connection newRetryConnection];
		}
		else if (statusCode == kHTTP_Unauthorized)
		{
#if 0
			// This is how I think it should work. HTTP unauthorized means you didn't sign something correctly or what?
			NSLog(@"Yahoo reports unauthorized for OAuth. Need to re-authorize.");
			[mDelegate deliciousAPIBadCredentials];
#else
			// This is just for testing
			NSLog(@"2 Yahoo reports unauthorized for OAuth. Need to re-authorize.");
			
			NSLog(@"trying to refresh access token again...");
			[self.yahooOAuthManager refreshAccessToken];
			NSLog(@"access token refresh returned");
			
			if ( !self.yahooOAuthManager.isOAuthLoggedIn )
			{
				NSLog(@"yahoo oauth manager reports oauth isn't logged in anymore. The user needs to re-authenticate");
				[mDelegate deliciousAPIBadCredentials];
			}
			[connection newRetryConnection];
#endif
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connectionObj
{
    ...
    NSLog(@"Succeeded! Received %d bytes of data", [[connection receivedData] length]);
...
- (BOOL)isUsingOAuth
{
	DXYahooOAuthManager *oauthManager = self.yahooOAuthManager;
	return oauthManager && [oauthManager isOAuthLoggedIn];
}
...
{
	struct tm t;
	char* result = strptime([timeString UTF8String], "%Y-%m-%dT%H:%M:%SZ", &t);
	
	if(result == NULL || *result != 0)
	{
		NSLog(@"Error parsing input string %@. Failed with %s", timeString, result);
		return nil;
	}
...
static BOOL DXRemoveDeliciousAPICredentials(void)
{
	...
	if(err != noErr)
		NSLog(@"Error removing item from keychain. SecItemDelete returned %d", err);
