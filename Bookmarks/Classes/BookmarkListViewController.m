...
{	
	if (editingStyle == UITableViewCellEditingStyleDelete) 
	{
		NSDictionary *postDictionary = [_postsArray objectAtIndex:indexPath.row];
		NSString *urlString = [postDictionary objectForKey:kDXPostURLKey];
		[[DXDeliciousAPIManager sharedManager] postDeleteRequest:urlString];
...
{
	// would pass in the post info here...
	[self showBookmarkInfo:nil];
}
...
- (void)viewWillAppear:(BOOL)animated
{
	...
	if([[DXDeliciousAPIManager sharedManager] isUpdating])
...
- (void)showBookmarkInfo:(id)sender
{
	[self.navigationController pushViewController:[[[BookmarkInfoViewController alloc] initWithPost:nil] autorelease] animated:YES];
}
