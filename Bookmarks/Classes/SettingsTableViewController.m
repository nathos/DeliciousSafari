...
- (id)initWithStyle:(UITableViewStyle)style {
    ...
    if (self = [super initWithStyle:style]) {
		...
		[[DXDeliciousAPIManager sharedManager] addObserver:self forKeyPath:@"isUserLoggedIn" options:NSKeyValueObservingOptionNew context:nil];
    }
    return self;
}
...
{	
	...
	if ([identifier isEqualToString:kUsernameCellIdentifier] || [identifier isEqualToString:kPasswordFieldCellIdentifier])
	{
		...
		rect = CGRectMake(LEFT_COLUMN_OFFSET, (cell.bounds.size.height - LABEL_HEIGHT) / 2.0, LEFT_COLUMN_WIDTH, LABEL_HEIGHT);
	...
	{			
		case kCredentialsSection:
		{
			switch(indexPath.row)
			{
				...
				case kCredentialsLoginLogoutRow:
				{
					if([[DXDeliciousAPIManager sharedManager] isUserLoggedIn])
					{
						// logout
						[[DXDeliciousAPIManager sharedManager] logout];
...
- (void)dealloc {
	[[DXDeliciousAPIManager sharedManager] removeObserver:self forKeyPath:@"isUserLoggedIn"];
...
- (void)textFieldDidEndEditing:(UITextField *)textField
{
	...
	else if(textField == _passwordTextField)
	{
		...
		[[DXDeliciousAPIManager sharedManager] clearSavedCredentials];
	}
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
	...
	if(object == [DXDeliciousAPIManager sharedManager])
	{
